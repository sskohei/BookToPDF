...
- テーマ切り替え（ライト/ダーク）: next-themes
- 多言語対応（日本語/英語）: 自作の軽量実装（`frontend/src/i18n/`、辞書 + React Context）
...
frontend/
  ...
  src/
    ...
    i18n/             # 日英切り替え（翻訳辞書 + LanguageProvider）
    lib/
...
- 新しいUI文言を追加する場合は `frontend/src/i18n/translations.ts` の `ja`/`en` 両方に追加する
  （日本語のみ、英語のみの文言を残さない）。
- 色は `frontend/app/globals.css` のCSS変数（`--text`, `--bg`, `--chip-fg` 等、`data-theme`属性で
  ライト/ダークが切り替わる）を使い、コンポーネントに新規の16進カラーをハードコードしない。
  Tailwindの `dark:` バリアントは `prefers-color-scheme` にしか反応せず `next-themes` の手動切り替えに
  追従しないため使用しない。
